...
#>
  ...
  $Filename = Split-Path $DocFilename -Leaf
  ...
  foreach($entry in $TOC){
    ...
    <#
    $atext = $($entry.Commandlet) -replace '-',''
    $anch = New-Object iTextSharp.text.Anchor $atext 
    $anch.Reference = $('#'+ $atext )
    $null=$anch.Add($dottedLine)
    $null=$anch.Add($entry.pages + $AddToPageNumber)
    $null=$p.add($anch)
    ...
    $atext = $($entry.Commandlet) -replace '-',''
    $Chunk = New-Object iTextSharp.text.Chunk $atext 
    #$null = $Chunk.SetLocalGoto($atext)
    #$null = $Chunk.SetAction( [iTextSharp.text.pdf.PdfAction]::LASTPAGE)
    $page = $($entry.pages + $AddToPageNumber)

    $null=  $Chunk.SetAction((new-object iTextSharp.text.pdf.PdfAction @(".\$Filename", $page)))
    $null = $p.Add($Chunk)
    $null=$p.Add($dottedLine)
    $null=$p.Add($entry.pages + $AddToPageNumber)

    #$Chunk = New-Object iTextSharp.text.Chunk "Link" 
    #$null = $p.Add($Chunk)
    $null = $Document.add($p);
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBEHuYwPX74GfXz
# uigULfR5aY5XbtnFw5FCNPv/lYUxWaCCGb8wggSEMIIDbKADAgECAhBCGvKUCYQZ
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIHbZ2Qr1SBcXxmTlUuk3eCpy
# U0NGCh2grMTH9/sYIqcFMA0GCSqGSIb3DQEBAQUABIIBAAErsdHf8fjw469mSqIu
# 4CHOxrDLbho3G6p9JGUq9kbdUuy3xQ4UaEUPRJKZ62asCZ5P0DqtqzacOe7Wvk/o
# URHua6Bje5BxgrSeLQKzFh00BR4d5tz2y9PHLa/1pOattBeCOOPsZGnIvZrSuGb8
# cNyeX5I1HJPRkkRrxdakUaPlW/HU9rt8/j8MEwR6Z4xCZFmcFT/U2JOFDZBRKi2W
# i9DHU6YlHGbOFzvFhEXyqXVeCWHeYtm4V/oL9Hr/78zQxFmwBOH7ZD4IU/yhjjIc
# ZWHngE0LSFBe9WpWDI+pnx6xqSwMRYpXoXMXj+bGTRaERNyuFXj6DxwYYn5UvcVf
# o6ahggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEwgY4wejELMAkGA1UEBhMC
...
# NjQ1MzlaMCMGCSqGSIb3DQEJBDEWBBR6y1ifQH7UMUVhY+LHJDvIkS/tFDANBgkq
# hkiG9w0BAQEFAASCAQADsUsUA50F9ye4TWtYsLm/qJ7DfJOLRAsMsKQO1JlsyJYQ
# UdU/3KeX7ZqVxjPG0jw2peZiKR+PuOQJnNSmpGPmvlseJQLyXrFW/66qdPhu8Kkq
# qBsHh8dLc/9SNmrDEziPcI3l1TpRfVWKe+6lTPWWsos5wCcbt4uvDuutvs63vOZN
# 7WMpLhuCPan9L+V9Vt5TEFTvFygt4MHP5inoUIsPwx6nZJzYdAs5KW8fxpKjskeF
# /6A/CHl6c4Y0dZ9shly8nGymDV5UiffIh+9+yadAMJGLL4MKtdxI3MCdnFK9WYee
# DuPE3irX8aj3jKQ+iTOn3h7gwwuXG+R5W48624i8
