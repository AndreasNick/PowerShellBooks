function Add-Headline{
  ...
  <#
  $AnchName = $text -replace "Cmdlet: ","" -replace " ","" -replace '-',''
  #Write-Host $( '"{0}"' -f $AnchName) -ForegroundColor Blue
  $p = New-Object -TypeName iTextSharp.text.Paragraph ""
  $anch = New-Object iTextSharp.text.Anchor "" 
  $anch.Name =   $AnchName 
  ...
  $AnchName = $text -replace "Cmdlet: ","" -replace " ","" -replace '-',''
  $p = New-Object -TypeName iTextSharp.text.Paragraph ""
  $Chunk = New-Object iTextSharp.text.Chunk $AnchName
  $null = $Chunk.SetLocalDestination($AnchName)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCdKGufQfl/S9iH
# MRt6/DZuzB3Uv2ntcxzcWMpepiaHgqCCGb8wggSEMIIDbKADAgECAhBCGvKUCYQZ
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIMCnvKvf8upQV4i11UTqYbgV
# os+LiSMmAmwuCrDBhCqHMA0GCSqGSIb3DQEBAQUABIIBAE2F1NNx9/0lBEq42iMV
# Hh0OdmmgVuheDQy+urtIiSz2MM7G+Ij94vtfCnwPGQq8FjGlBTuOLsT1KyBkjYwt
# 6XuQPWGvkB9vaQLYmuXQTqFgJIB4+GubPxX1Do00xbq2TuAk2jauKhCwL8n+/rGM
# pJ7KYRhuLDK6+9vCSYJPcLVtXt0LdGf6GlE9nyPiGzsuVlWoVTlAPodjPloFz0kd
# g3w2VrhBK2krBADK93WJx7BtdrlHysU0+d8ZzFTJgQCLvsCXQQpUg9PiTF+5AVQ1
# A/RNk09DK3RLAp3Mcc4UZpxKA1c3QcP0X1x2Mg6H5j+5tVYjJLww1Mua/ghfzCkR
# L66hggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEwgY4wejELMAkGA1UEBhMC
...
# NjQ0NTNaMCMGCSqGSIb3DQEJBDEWBBTea5LMgTOHO4sq4HG+yBqMa1rm7DANBgkq
# hkiG9w0BAQEFAASCAQAeo2rfpNa2J96fGRkfp97OaD4c+qQJUgpZprCNgFia0Y8/
# 81xpzIjhZQZCTSuMGx0wYzgrDhSCybFDp+rZaamSVOI7PQPC558johpiStOq88bl
# eDwGA3cVwEcCfBwRblYmi5EZb5q7Hs5rRXQAtolfKFCCC1+3g8mLk/EywXkaSgaH
# 1+ZHDStzRRm5A9dLYG0u0zHGlKugKfkbPtZLUiF/Aq7SdAwnUFlT3cqy/T5ZNDcm
# jZPeA20cROdsMaNJPcTrjcJvb5Rl2EWThs6O9Lu7Pt/U2d6zkJbCs7381h0IjvLX
# svUpGFaHhc0kYh+qHegYl2EZNGnCs6MMnAE9G3WU
