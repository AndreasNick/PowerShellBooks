...
function New-PowerShellBook {
  ...
  process {
    ...
    foreach($command in $commandlets){
      ...
      Write-Progress -Activity "Create Chapter pdf" -status "Create file $OutFile"  -percentComplete (($i / ($commandlets.count))*100) 
      $i++
    }
    Write-Progress -Completed -Activity "make progress bar dissapear"


    $Pages = New-TableOfContent -OutputPdfDocument $($TempFolder+'\toc.pdf') -TOC $Indexliste -AddToPageNumber 3 -DocFilename $OutputPdfDocument
    
    if($pages -ne 1){
      New-TableOfContent -OutputPdfDocument $($TempFolder+'\toc.pdf') -TOC $Indexliste -AddToPageNumber (2 + $pages) -DocFilename $OutputPdfDocument
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB9W0NcdoDMfyT6
# psikYxcIJFn7dGkw07VlDAjZIU8ayqCCGb8wggSEMIIDbKADAgECAhBCGvKUCYQZ
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIGcZbkc1aOPj5iYQ0bbnLB9O
# b3esW7/sXbz0L0nXyPe5MA0GCSqGSIb3DQEBAQUABIIBAGtyQP41W92uDcZCxnkU
# 5Pv2exDtQAQHIEe8rUP9obs/L7UqQaf4zW+1ZWkdepnW80gMcXfLZXr94FlRt0cm
# yNug/frraOQLRwFZNu5n1vc/zi2rtk3gV2mBNjfeAJi6FvXOeIC/bJtLPaZBH9OM
# LSb96O3n9JDYc26FD4nDaD0JuEcpTsrzlQ93Kijivgd9rwoNx8Vaf4su6SmE3DRg
# D6AHOXpEXMMMfVWU4kWh25r5058JEmIgjQ4/6HoCxEGelP6teJ6xfz2vGQgKH2q9
# 2c8r0/2EMuBo7VDTiuoPsQLD5VIuYBweHg0fk+tb/eKMjsX12BGB+N8MAgkHRURQ
# g3GhggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEwgY4wejELMAkGA1UEBhMC
...
# NjQ1MzNaMCMGCSqGSIb3DQEJBDEWBBTDgPpl2z1O3cIYb32u1cI0G6F2IDANBgkq
# hkiG9w0BAQEFAASCAQBlfxCz81Ow8jgVrkdUfgfhuSXQgIuKuuKGSX6fYrJIm4Jj
# GBEPKecvl9qZNzbD8CkTKE/ApXToHFle4No+vP0ocvWVNXoGOBaSRXcdk1G3oHX3
# Y7zX/L9wTCMmi+1GBbA2bcLSt/UprQx/r0kgyfLny5u34yK6P2kMXWA3K2xthriL
# UAghbKSZcSoqO3O3ph1honMiT7hMzdx0FsQ/GFmrmDkuHdMo19lo2DAknE12oHFh
# vNKQWggUjIm20tN2g1glKzB+5fIceDEdFIQ9cRDvc/ULsvG0dCkagvpYr5BLZfZf
# hLk1iIZCR87Ijr/QQGsKrUIWcCm0iTLe4t+1kaqN
